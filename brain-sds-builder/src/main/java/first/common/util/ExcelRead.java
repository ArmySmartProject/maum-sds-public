...
public class ExcelRead {
  ...
  public static List<Map<String, String>> read(ReadOption excelReadOption) {
    //엑셀 파일 자체
    //엑셀파일을 읽어 들인다.
    //FileType.getWorkbook() <-- 파일의 확장자에 따라서 적절하게 가져온다.
    Workbook wb = FileType.getWorkbook(excelReadOption.getFilePath());
    ...
    for(int i =0; i<sheetNum; i++){
      ...
      for(int rowIndex = excelReadOption.getStartRow() - 1; rowIndex < numOfRows; rowIndex++) {
        ...
        if(row != null) {
           ...
           */
          numOfCells = 26; //한개의 행마다 몇개의 cell이 있는지 체크
          ...
          Map<String, String> sheetType = new HashMap<>();

          if (cnt == 1) {
            sheetType.put("A", "Sheet_Change");
            result.add(sheetType);
            cnt = 0;
          }
          /*
           * cell의 수 만큼 반복한다.
          ...
          for(int cellIndex = 0; cellIndex < numOfCells; cellIndex++) {
             ...
             */
            map.put(cellName, CellRef.getValue(cell, wb));
          }
          /*
