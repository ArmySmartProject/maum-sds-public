...
function drawFlowChart(jsonData, lang) {
  ...
  try {
    console.log("<drawFlowChart init json data v2>");
    console.log(data);
    ...
    if (isInitData && sourceList.length > 0) {
      ...
      while (data.nodes.length > checkedNodeList.length) {
        var nodeId;
        for (var nodeIdx in data.nodes) {
          if (!checkedNodeList.includes(data.nodes[nodeIdx].id)) {
            nodeId = data.nodes[nodeIdx].id;
            break;
          }
        }
        sourceList.push(makeNodeList(nodeId));
      }
    ...
    var globalNodeTop = 150;
    data.nodes.forEach(node => {
      if (node.type === 'global') {
    	globalNodeTop -= 100;
        node.top = globalNodeTop;
        node.left = lastLeft + 400;
...
var nodeClickEvent = function(params) {
  ...
  if (nodeData['attr'].length > 0) {
    ...
    var nodeIntentList = nodeAttr.intentList
  ...
  var innerHtml = "";
  if(nodeIntentList != null){
	  for (var i = 0; i < nodeIntentList.length; i++) {
		  if(nodeIntentList[i].intent != null && nodeIntentList[i].intent != ""){
			  innerHtml += "<div class='user_content'>";
			  innerHtml += "<div class='sub_tit'><span>"+ nodeIntentList[i].intent +"</span></div>";
			  innerHtml += "<a href='#chat_speech_delete' class='btn_icon delete btn_lyr_open'></a>";
			  innerHtml += "<div class='description'>";
			  innerHtml += "<p class='textarea' id='"+ nodeIntentList[i].intent +"_say'>"+nodeIntentList[i].answer+"</p>";
			  innerHtml += "</div>";
			  innerHtml += "</div>";
		  }
	  }
  ...
  for(var key in nodeData['attr'][0]){
	  if(key == 'utterY'){
		  ...
		  innerHtml += "<div class='user_content'>";
		  innerHtml += "<div class='sub_tit'><span>"+ key.split('utter')[1] +"</span></div>";
		  innerHtml += "<a href='#chat_speech_delete' class='btn_icon delete btn_lyr_open'></a>";
		  innerHtml += "<div class='description'>";
